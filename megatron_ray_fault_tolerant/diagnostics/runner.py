...
class DiagnosticConfig:
    ...
    # Simulation mode - when True, tests always pass
...
class DiagnosticRunner:
    ...
    def _run_all_phases(self, actors: List) -> DiagnosticResult:
        ...
        gpu_results = ray.get(
            [a.run_gpu_health_check.remote(sim) for a in actors],
            timeout=self.config.timeout,
        )
        ...
        intra_results = ray.get(
            [a.run_intra_node_comm_test.remote(sim) for a in actors],
            timeout=self.config.timeout,
        )
        ...
        inter_results = ray.get(
            [a.run_inter_node_comm_test.remote(sim) for a in actors],
            timeout=self.config.timeout,
        )
