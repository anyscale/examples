...
def init_megatron_optim_config(optim_config) -> OptimizerConfig:
    optim_args = {
        "optimizer": optim_config.get("optimizer", "adam"),
        "lr": optim_config.get("lr", 1.0e-6),
...
def get_megatron_optimizer_param_scheduler(
    ...
    """
    lr_warmup_steps = config.get("num_warmup_steps", 0)
    ...
    if config.get("lr_warmup_steps_ratio", None) is not None and (
        config.get("lr_warmup_steps", None) is None
        or config.get("lr_warmup_steps", 0) <= 0
    ):
        lr_warmup_steps = int(config.get("lr_warmup_steps_ratio", 0.0) * lr_decay_steps)

    opt_param_scheduler = OptimizerParamScheduler(
        optimizer,
        init_lr=config.get("lr_warmup_init", 0.0),
        max_lr=config.get("lr", 1.0e-6),
        min_lr=config.get("min_lr", 0.0),
        lr_warmup_steps=lr_warmup_steps,
        lr_decay_steps=lr_decay_steps,
        lr_decay_style="constant",
        start_wd=config.get("weight_decay", 0.01),
        end_wd=config.get("weight_decay", 0.01),
