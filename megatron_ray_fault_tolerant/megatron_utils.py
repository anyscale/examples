...
def offload_megatron_grads_to_cpu(models):
            ...
            for buffers in model_chunk_all_buffers:
                for buffer in buffers:
                    if buffer.grad_data.storage().size() > 0:
                        buffer_sizes.append(buffer.grad_data.storage().size())
                        buffer.grad_data.storage().resize_(0)
            all_buffer_sizes.append(buffer_sizes)
        else:
    ...
    return all_buffer_sizes


@torch.no_grad()
def load_megatron_grads_to_gpu(models, buffer_sizes):
