...
# Limit the number of rows per epoch. Set to None for a full run.
LIMIT_ROWS = 2000
...
def train_loop_per_worker(config: dict):
    ...
    if checkpoint:
        start_epoch, step = util.load_checkpoint(checkpoint, policy, optimizer)
    else:
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # optimizer state); the others just report metrics.
        #
...
#   num_workers=8  -> 8 GPU workers, each running train_loop_per_worker
